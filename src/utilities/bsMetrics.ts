...
function initializeMetricsTransport()
{
    ...
    }, BSWhatsNewConstants.REGISTRATION_METRICS_FLUSH_MS);
...
async function flushMetricsQueue(reason: 'interval' | 'hidden' | 'unload' | 'batch-size')
{
    ...
    try
    {
        ...
        {
            const chunk = metricsQueue.slice(0, BSWhatsNewConstants.REGISTRATION_METRICS_BATCH_SIZE);
...
{
    if (!BSWhatsNewConstants.USE_REGISTRATION_METRICS) return;
    ...
    if (metricsQueue.length >= BSWhatsNewConstants.REGISTRATION_METRICS_BATCH_SIZE)
    {
